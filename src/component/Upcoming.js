...
function Upcoming() {
    ...
    const api_url = `https://api.themoviedb.org/3/discover/movie?api_key=${api}&language=te-IN&region=IN&with_original_language=hi&with_genres=878`;
    ...
    const handleClick = (details) => {
        if (!trailer) {
            searchYouTubeVideo(details?.title || "");
        } else {
            setTrailer("");
        }
    }
    ...
    return (
        <>
            <section className='pop-con'>
                <p className='popular-text'>Upcoming</p>
