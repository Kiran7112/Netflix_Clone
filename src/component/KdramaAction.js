...
function  KrDramaAction () {
    ...
    const api_url = `https://api.themoviedb.org/3/discover/tv?api_key=${api}&language=ko-KR&region=KR&with_original_language=ko&with_genres=10759`;
    ...
    return (
        <>
            <section className='pop-con' id='koreon'>
                <p className='popular-text'>Most Watched Exciting KDrama </p>
                <div className='pop-box-cont'>
                    {popsearch.map((details, index) => (
                        <div key={index} className="pop_box" style={{backgroundImage: `url(https://image.tmdb.org/t/p/w500/${details.poster_path})`, backgroundSize: 'cover',objectPosition:'center',backgroundPosition:'center'}}>
                           ...
                           <p className='text-center '>{details.original_name}</p>
                            </div>
                        </div>
